...
import { serverCartApi, serverUserApi, isAuthenticated } from "@/lib/api/server"
...
export async function Header() {
  ...
  const authenticated = await isAuthenticated(cookieStore, headersList)

  // Only fetch profile and cart if user is authenticated
  const [profileResponse, cartResponse] = await Promise.all([
    authenticated ? serverUserApi.getProfile() : Promise.resolve({ success: false, data: null }),
    authenticated ? serverCartApi.getCart() : Promise.resolve({ success: false, data: null }),
  ])

  const user = (profileResponse.success ? profileResponse.data : null) as User | null
  const isAuth = !!user
  const role = (user?.userType || "").toUpperCase()
  const cartItems = cartResponse.success && Array.isArray(cartResponse.data) ? cartResponse.data : []
  const cartCount = cartItems.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0)

  const chatPath = !isAuth
    ? "/login?redirect=/messages"
    : role === "SELLER" || role === "ADMIN" || role === "SHIPPER"
      ? "/seller/messages"
      : "/messages"
      ...
      <div className="bg-primary text-primary-foreground">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between py-2 text-sm">
            ...
            <div className="flex items-center gap-4">
              <Link href={chatPath} prefetch={false} className="flex items-center gap-1 hover:opacity-80">
              ...
              <HeaderUserMenu initialUser={user} initialIsAuthenticated={isAuth} />
            </div>
          </div>
        </div>
      </div>
