...
import { SellerOverview } from "@/lib/api/seller"
...
export function SellerDashboardClient({ initialOverview, initialOrders }: SellerDashboardClientProps) {
  const [showAddProduct, setShowAddProduct] = useState(false)
  const [overview] = useState<SellerOverview | null>(initialOverview)
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-green-600 mt-1">{overview?.revenueChange || "+0%"} so với tháng trước</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{overview?.newOrders ?? 0}</div>
                <p className="text-xs text-green-600 mt-1">{overview?.newOrdersChange || "+0%"}</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{overview?.productsCount ?? 0}</div>
                <p className="text-xs text-green-600 mt-1">{overview?.productsChange || "+0"}</p>
              </CardContent>
            </Card>
            <Card>
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <CardContent>
                <div className="text-2xl font-bold">{overview?.views ?? 0}</div>
                <p className="text-xs text-green-600 mt-1">{overview?.viewsChange || "+0%"}</p>
                </CardContent>
              </Card>
          </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                          ...
                          <tr key={order.id} className="border-b hover:bg-accent/5">
                              ...
                              <td className="py-3 px-4 font-medium">{formatCurrency(order.finalTotal || order.totalPrice || 0)}</td>
